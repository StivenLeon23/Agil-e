...
router.post ('/login', async (req,res)=>{
    ...
    if(!passValida) return res.status(400).json({error:true, mensaje: 'usuario o contraseña invalido'})
...
router.post('/register', async(req,res)=>{
    ...
    if (existeEmail) return res.status(400).json({error: true, mensaje: 'usuario ya registrado'})
